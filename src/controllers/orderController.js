...
class OrderController {
  ...
  async createOrder(req, res) {
    ...
    if (!deliveryLocation || !deliveryLocation.type || !deliveryLocation.coordinates) {
      const resp = error("Delivery location is required and must be a GeoJSON Point", 400);
  ...
  async getMyOrders(req, res) {
    ...
    cache.set(cacheKey, resp, 60); // cache 60 seconds
  ...
  async updateOrderStatus(req, res) {
    ...
    if (!allowedTransitions[order.status]?.includes(status)) {
      const resp = error(`Invalid status transition from ${order.status} to ${status}`, 400);
      return res.status(resp.status).json(resp);
    }
